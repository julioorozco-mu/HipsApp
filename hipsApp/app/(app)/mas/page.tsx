...
export default async function MorePage() {
  ...
  const menuItems = canManageOperations(role) ? managerItems : [];
    ...
    <main className="min-h-dvh bg-[oklch(0.965_0.018_300)] p-2 sm:p-5">
      <div className="mx-auto flex min-h-[calc(100dvh-1rem)] w-full max-w-lg flex-col overflow-hidden rounded-[2.5rem] bg-card shadow-[0_18px_50px_oklch(0.25_0.04_300/0.14)] sm:min-h-[calc(100dvh-2.5rem)]">
        <div className="flex-1 px-5 pt-7 pb-7 sm:px-8 sm:pt-12">
          ...
          <Link
            href="/perfil"
            className="mt-7 grid min-h-28 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 rounded-3xl border px-5 py-4 hover:bg-secondary"
          >
            ...
            <span className="min-w-0">
              ...
              <span className="mt-1 block text-sm text-muted-foreground">{roleLabel(role)}</span>
            </span>
          ...
          {menuItems.length ? (
            <nav
              ...
              className="mt-7 divide-y overflow-hidden rounded-3xl border"
            >
              {menuItems.map(({ href, icon: Icon, label }) => (
                <Link
                  key={href}
                  href={href}
                  className="grid min-h-17 grid-cols-[auto_1fr_auto] items-center gap-4 px-5 py-3 font-semibold hover:bg-secondary"
                >
